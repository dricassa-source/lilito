...
function Lembretes() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await applyScope(supabase.from("lembretes").select("*, prospects(nome)"), scopeIds)
        .order("concluido")
        .order("data")
        .order("hora");
...
function LembreteRow({ l, onToggle }: { l: any; onToggle: (v: boolean) => void }) {
  return (
    <div className="p-3 flex items-center gap-3">
      <Button
        variant={l.concluido ? "default" : "outline"}
        size="icon"
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <div className="space-y-3">
          <div>
            <Label>Título</Label>
            <Input value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Ex.: Ligar para Dr. João" />
          </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={() => titulo && save.mutate()} disabled={save.isPending}>
            Salvar
          </Button>
        </DialogFooter>
      </DialogContent>
